...
export default function StockPrepDropzone() {
  ...
  const handleRunPrep = async () => {
    ...
    setError(null);
    setProgressMessage('Prepping stock exports (Auto-clean, EXIF sanitize, 95% JPEG)...');
    try {
      const targetDir = outputDir || (localPaths[0] ? localPaths[0].substring(0, localPaths[0].lastIndexOf('\\')) : '');
      const res: any = await handleStartStockPhotoPrep(localPaths, targetDir);
      setResultSummary(`Successfully exported ${localPaths.length} stock-compliant files.`);
      toast.success(`Stock Photo Prep Complete: ${localPaths.length} files saved!`);
    } catch (err) {
      setError(String(err));
    ...
    <div className="flex flex-col gap-3 p-3 bg-surface/50 rounded-xl border border-border-color/30">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-[10px] text-text-secondary">Commercial QA & Clean EXIF Export</div>
          </div>
        </div>
        {localPaths.length > 0 && (
          <button
      ...
      >
        ...
        <span className="text-[10px] text-text-secondary text-center">
          Drop RAW/JPEG files to clean noise, remove dust, and strip GPS/private tags
        </span>
      </div>
      ...
      {resultSummary && (
        <div className="flex items-center gap-1.5 p-2 rounded bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 text-xs">
      ...
      >
        {isProcessing ? (
          ...
          <>
            <Sparkles size={14} />
            <span>Prep {localPaths.length} Photos for Stock</span>
          </>
        )}
      </button>
    </div>
