...
function parseDms(dmsValue: any) {
  if (dmsValue === null || dmsValue === undefined) {
    return null;
  }
  if (typeof dmsValue === 'number') {
    return Number.isFinite(dmsValue) ? dmsValue : null;
  }
  const dmsString = String(dmsValue);
  const parts = dmsString.match(/(\d+\.?\d*)\s+deg\s+(\d+\.?\d*)\s+min\s+(\d+\.?\d*)\s+sec/);
  if (!parts) {
    const numeric = parseFloat(dmsString);
...
export default function MetadataPanel({ selectedImage }: MetaDataPanelProps) {
  const { keyCameraSettings, gpsData, otherExifEntries } = useMemo(() => {
    const exif = selectedImage?.exif && typeof selectedImage.exif === 'object' ? selectedImage.exif : {};
  ...
  const hasGps = Number.isFinite(gpsData.lat) && Number.isFinite(gpsData.lon);
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    {gpsData.altitude !== null && gpsData.altitude !== undefined && (
                      <MetadataItem label="Altitude" value={gpsData.altitude} />
                    )}
                  </div>
                </div>
              </div>
